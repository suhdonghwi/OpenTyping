...
namespace OpenTyping
{
    ...
    public class MathConverter : MarkupExtension, IValueConverter
    {
        ...
        private static MathConverter _converter = null;
        public override object ProvideValue(IServiceProvider serviceProvider)
        {
            if (_converter == null) _converter = new MathConverter();
            return _converter;
        }
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            // Consider conversion culture! Decimal deliminater should be changed to the Dot for right calc.
            mathEquation = mathEquation.Replace(",", "."); 
            ...
            while (token != string.Empty)
            {
                ...
                if (_operators.Contains(token))
                {
                    ...
                    if (numbers.Count > (index + 1) &&
                        (double.Parse(nextToken, NumberStyles.Any, CultureInfo.InvariantCulture) == numbers[index + 1] || nextToken == "("))
                    {
                        switch (token)
                        {
                            case "+":
